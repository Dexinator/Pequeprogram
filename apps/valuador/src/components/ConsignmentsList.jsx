...
const ConsignmentsList = () => {
  ...
  // Cada fila es un valuation_item, mientras que ContratoConsignacion espera un
  // arreglo de productos + los datos del cliente, así que se moldea aquí.
  const reprintContract = (consignment) => {
    setContractData({
      ...
      // Fecha del contrato original si existe; si no, la de creación del registro
  ...
  console.log('📊 Stats values:', {
    total_unpaid_value: stats.total_unpaid_value,
    total_paid_value: stats.total_paid_value,
    total_available_value: stats.total_available_value
  });
